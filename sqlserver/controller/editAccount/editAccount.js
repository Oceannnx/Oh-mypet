...
const editAccount = async (req, res) => {
  try {
    ...
    await client.query(
      `update user set fName = "${fName}", lName = "${lName}", email = "${email}",profileImg = "${profileImg}", tel = "${tel}", line = "${line}", facebook = "${facebook}", twitter = "${twitter}", instagram = "${instagram}", address = "${address}" where _id = "${req.cookies.userID}"`,
    )
